...
function App() {
  ...
  async function connect(item: Connection) { setLoading(true); try { await window.sqlConnect.db.connect(item); setConnected(v => [...new Set([...v, item.id])]); setExpanded(v => ({ ...v, [item.id]: true })); const saved: any = { ...item }; delete saved.create; delete saved.password; setConnections(v => v.some(c => c.id === saved.id) ? v.map(c => c.id === saved.id ? saved : c) : [...v, saved]); const nextConnections = connections.some(c => c.id === saved.id) ? connections.map(c => c.id === saved.id ? saved : c) : [...connections, saved]; await window.sqlConnect.settings.save(nextConnections); if (item.type === 'mysql') { const names = await window.sqlConnect.db.databases(item.id); setMysqlDatabases(v => ({ ...v, [item.id]: names })); const db = item.database || names[0]; if (db) { setSelectedDatabase(v => ({ ...v, [item.id]: db })); await loadSchema(item.id, db) } } else { const items = await window.sqlConnect.db.schema(item.id); setSchema(v => ({ ...v, [item.id]: items })) } notify(`已连接 ${item.name}`, 'success'); return true } catch (error) { if (item.type === 'mysql' && !item.password) { setEditingMySQLId(item.id); setMysqlForm({ name: item.name, host: item.host, port: String(item.port), user: item.user, password: '', rememberPassword: !!item.rememberPassword, tls: item.tls !== false, caPath: item.caPath || '' }); setShowMySQL(true) } notify(error instanceof Error ? error.message : String(error), 'error'); return false } finally { setLoading(false) } }
