...
const api: SqlConnectApi = {
  settings: { load: () => ipcRenderer.invoke('settings:load'), save: (connections) => ipcRenderer.invoke('settings:save', connections), remove: (connectionId) => ipcRenderer.invoke('settings:removeConnection', connectionId) },
