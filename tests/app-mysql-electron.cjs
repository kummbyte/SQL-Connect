...
app.whenReady().then(async () => {
  ...
  try {
    ...
    for (let index = 0; index < 18; index++) await js('document.querySelector(".new-query").click()')
    await until(async () => (await activeTabMetrics())?.scrollWidth > (await activeTabMetrics())?.clientWidth && (await activeTabMetrics())?.visible, 'new MySQL query tabs scroll into view')
    ...
    await until(async () => (await activeTabMetrics())?.visible, 'reactivating the same MySQL tab reveals it')
    const mysqlLongTable = 'this_is_a_very_long_table_name_for_sidebar_layout_checks'
    await js(`Array.from(document.querySelectorAll('.table-object-button')).find(button => button.textContent.includes(${JSON.stringify(mysqlLongTable)})).click()`)
    await until(() => js('document.querySelector(".tab.active")?.textContent.includes("' + mysqlLongTable + '")'), 'opening the MySQL table from sidebar activates it')
    await until(async () => (await activeTabMetrics())?.visible, 'MySQL sidebar table tab scrolls into view')
    await js('document.querySelector(".tabbar").scrollLeft = 0')
    await delay(100)
    assert.equal((await activeTabMetrics()).scrollLeft, 0)
    await js(`Array.from(document.querySelectorAll('.table-object-button')).find(button => button.textContent.includes(${JSON.stringify(mysqlLongTable)})).click()`)
    await until(async () => (await activeTabMetrics())?.visible, 'reopening the active MySQL table reveals its tab')
    await js('document.querySelector(".tab.active svg:last-child").dispatchEvent(new MouseEvent("click", { bubbles: true }))')
    await until(async () => (await activeTabMetrics())?.visible, 'closing active MySQL tab reveals its neighbor')
    console.log('PASS: overflowing MySQL tab bar reveals active query and sidebar table tabs while preserving manual scrolling')
