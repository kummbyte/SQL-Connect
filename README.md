...
分别覆盖请求生命周期（退出、超时、异常消息）、真实 Electron utility process 的 SQLite 和本机 MySQL 8.4 通信，以及 BrowserWindow 中连接 MySQL、选择数据库、首次点击表和 SQL 编辑器快捷键的完整流程。配置测试覆盖 SQLite 符号链接重复合并、只读保留、MySQL 不同用户区分、配置备份、原子保存和按连接 ID 删除。MySQL 测试会在临时数据目录和端口启动隔离实例，创建临时数据库和账号后自动清理，不读取用户连接配置。界面测试还覆盖“新建连接”菜单、重复打开 SQLite/MySQL、SQLite 子菜单、MySQL 表单取消、文件选择取消、创建失败后恢复和重试、按行断开并重连、确认后删除在线或离线连接且保留数据库文件、MySQL 连接信息折叠、数据库树对齐及多数据库节点独立展开，以及 SQLite/MySQL 窄侧栏中的长表名和视图行布局、固定图标尺寸、列间无重叠和主题切换，以及 Tab 补全和 ⌘ Enter 执行。
