...
- `npm run test:app`：先构建，再验证“新建连接”菜单、SQLite 子菜单、MySQL 表单取消、Esc 关闭、创建数据库、保存连接、失败提示、文件选择取消和重试，在线/离线连接删除确认与数据库文件保留，SQLite 长表名和视图在窄侧栏下的图标尺寸与列间距，以及 SQL 编辑器的 Tab 补全和 ⌘ Enter 执行。文件选择器返回值由测试替身提供，不能据此宣称原生对话框交互已人工验收。
- `npm run test:app:mysql`：在隔离 MySQL 8.4 实例中验证多数据库表浏览、同名表上下文、多个数据库节点独立展开和收起、默认查询数据库选择、重复连接复用、无密码重连提示、连接信息独立折叠、数据库树根级对齐、在线删除，MySQL 长表名和视图在窄侧栏下的图标尺寸与列间无重叠，以及 SQL 编辑器的 Tab 补全和 ⌘ Enter 执行。
...
- SQLite 与 MySQL 共用表行样式：表图标固定为 16px，表名区域可收缩并省略，`TABLE/VIEW` 标识和结构按钮保持独立宽度，避免长标识符覆盖右侧控件。
- 表名完整内容通过按钮悬停提示提供；测试在窄侧栏中测量长表名、视图的图标尺寸和各列边界，并检查主题切换后的尺寸。
